...
public class ReservationExcursionServiceImpl implements CrudService<ReservationExcursion, Long> {

    private final Connection cnx = MyDBConnexion.getInstance().getConnection();

    // --- RÉCUPÉRATION PAR ID DE RÉSERVATION ---
    ...
    public void updateFullExcursion(Reservation res, ReservationExcursion re) {
        ...
        try {
            ...
            ps2.setString(5, re.getHeure_souhaitee()); // Ajout de l'heure
    ...
    public void createFullExcursion(Reservation res, ReservationExcursion re) {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                // 2. Insertion dans 'reservation_excursion'
                PreparedStatement ps2 = cnx.prepareStatement(sqlExc);
                ...
                ps2.setString(6, re.getHeure_souhaitee()); // Ajout de l'heure
                ps2.executeUpdate();

                cnx.commit();
                System.out.println("✅ Réservation complète enregistrée !");
            }
        } catch (SQLException e) {
            try { cnx.rollback(); } catch (SQLException ex) { ex.printStackTrace(); }
            e.printStackTrace();
    ...
    private ReservationExcursion mapResultSetToEntity(ResultSet rs) throws SQLException {
        ...
        re.setHeure_souhaitee(rs.getString("heure_souhaitee")); // Récupération de l'heure
        return re;
    }
    ...
    @Override
    public ReservationExcursion create(ReservationExcursion re) {
    ...
    @Override
    public ReservationExcursion update(ReservationExcursion re) {
    ...
    @Override
    public ReservationExcursion findById(Long id) {
    ...
    @Override
    public List<ReservationExcursion> findAll() {
