...
public class ReservationRestaurantServiceImpl implements CRUD<ReservationRestaurant, Long> {
    ...
    /**
     * Finds the child record using the parent Reservation ID
     */
    ...
    /**
     * Atomic Update: Updates 'reservation' AND 'reservation_restaurant' tables
     */
    public void updateFullReservation(Reservation res, ReservationRestaurant rr) {
        ...
        try {
            ...
            // 1. Update Parent
            try (PreparedStatement ps1 = cnx.prepareStatement(sqlRes)) {
            ...
            // 2. Update Child
            try (PreparedStatement ps2 = cnx.prepareStatement(sqlResto)) {
    ...
    /**
     * Atomic Creation: Inserts Parent then Child using the generated ID
     */
    public void createFullReservation(Reservation res, ReservationRestaurant rr) {
        ...
        try {
            ...
            // 1. Parent Insertion
            try (PreparedStatement ps1 = cnx.prepareStatement(sqlRes, Statement.RETURN_GENERATED_KEYS)) {
                ps1.setDouble(1, res.getPrix_total());
                ps1.setString(2, res.getStatut().name());
                ps1.setString(3, res.getType_res());
                ps1.setDate(4, java.sql.Date.valueOf(res.getDate_debut()));
                ps1.setDate(5, java.sql.Date.valueOf(res.getDate_debut()));
    ...
    @Override
    public ReservationRestaurant insert(ReservationRestaurant rr) { // Renamed from create
    ...
    @Override
    public ReservationRestaurant getById(Long id) { // Renamed from findById
    ...
    @Override
    public List<ReservationRestaurant> selectAll() { // Renamed from findAll
