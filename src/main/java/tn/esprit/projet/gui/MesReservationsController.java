...
import javafx.scene.layout.BorderPane; // 👈 Zid hadhi
import javafx.geometry.Insets;         // 👈 Zid hadhi
...
public class MesReservationsController {
    ...
    private void switchToPaymentView(String url, String sessionId, Reservation res) {
        // 1. Fel FXML mte3ek esmou "mainLayout"
        BorderPane mainLayout = (BorderPane) tableMesReservations.getScene().lookup("#mainLayout");

        if (mainLayout == null) {
            System.err.println("❌ Erreur: mainLayout introuvable !");
            return;
        }

        // 2. On prépare le WebView
        ...
        // Garder une trace de l'ancienne vue (le ScrollPane)
        Node oldCenter = mainLayout.getCenter();
        btnCancel.setOnAction(e -> mainLayout.setCenter(oldCenter));

        engine.locationProperty().addListener((obs, oldUrl, newUrl) -> {
            if (newUrl.contains("/success")) {
                Platform.runLater(() -> {
                    mainLayout.setCenter(oldCenter);
                    verifierStatusStripe(sessionId, res);
                });
            } else if (newUrl.contains("/cancel")) {
                Platform.runLater(() -> mainLayout.setCenter(oldCenter));
            }
        });

        VBox layoutPaiement = new VBox(10, btnCancel, webView);
        layoutPaiement.setPadding(new Insets(20));

        // 3. Injecter dans le centre du mainLayout
        mainLayout.setCenter(layoutPaiement);
        engine.load(url);
    }
    ...
    private void handleModifierAction(Reservation res) {
        try {
            String type = res.getType_res().toUpperCase();
            String fxml = type.equals("RESTAURANT") ? "/RestaurantBookingView.fxml" : "/ExcursionBooking.fxml";

            FXMLLoader loader = new FXMLLoader(getClass().getResource(fxml));
            Parent root = loader.load();

            if (type.equals("EXCURSION")) {
                ExcursionBookingController controller = loader.getController();
                ReservationExcursionServiceImpl resExcService = new ReservationExcursionServiceImpl();

                // ✅ Fix: findByReservationId
                ReservationExcursion re = resExcService.findByReservationId(res.getId());

                ExcursionService excService = new ExcursionService();
                Excursion exc = excService.getById(re.getExcursion_id().intValue());

                // 🔥 Hedhi el méthode elli b3atht'ha enti tawa, mrigla!
                controller.setReservationData(res, re, exc);
            }

            // 🚀 NAVIGATION FIX:
            // 1. N-jarbou nlawjou 3la blassa dakhil el dashboard (ken el menu dima 3al imin/issar)
            Node contentArea = tableMesReservations.getScene().lookup("#contentArea");

            if (contentArea instanceof Pane) {
                ((Pane) contentArea).getChildren().setAll(root);
            } else {
                // 2. Ken mal9ach contentArea, n-badlou el khedma dakhil el Scene root direct
    ...
    private void confirmerAnnulation(Reservation res) {
        ...
        alert.showAndWait().ifPresent(response -> {
            if (response == ButtonType.YES) {
                try {
                    // On transforme le Long en int ici pour que le service soit content
                    int idAsInt = res.getId().intValue();

                    service.delete(idAsInt);
                    masterData.remove(res);
