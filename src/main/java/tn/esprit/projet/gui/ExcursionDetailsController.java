...
public class ExcursionDetailsController implements Initializable {
    ...
    private Excursion selectedExcursion; // Variable globale dans le controller
    @FXML
    ...
    public void setExcursionData(Excursion e) {
        ...
        this.selectedExcursion = e; // <--- TRÈS IMPORTANT
        int currentReservations = excursionService.getCurrentReservations(e.getId());
        double dynamicPrice = excursionService.calculateDynamicPrice(e, currentReservations);
        this.currentExcursionPrice = dynamicPrice;
    ...
    private void handleOpenBooking() {
        try {
            ...
            // Passer l'excursion actuelle au contrôleur de réservation
            ExcursionBookingController bookingController = loader.getController();

            // selectedExcursion lezem tkoun 3andek fel controller (khzann'ha f setExcursionData)
            bookingController.setExcursionData(this.selectedExcursion);

            // Injection dans le mainLayout du dashboard
            BorderPane mainLayout = (BorderPane) nameLabel.getScene().lookup("#mainLayout");
            if (mainLayout != null) {
                mainLayout.setCenter(view);
            } else {
                // Fallback si lookup fchel
                SceneManager.loadClientContent("/ExcursionBooking.fxml");
            }

        } catch (IOException e) {
            System.err.println("❌ Erreur lors de l'ouverture du booking : " + e.getMessage());
            e.printStackTrace();
        }
    }
